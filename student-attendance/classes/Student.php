...
class Student{
	...
	public function getStudents(){
		...
		$query = "SELECT *  FROM tbl_student s, classmst c where s.classid=c.classid ";
	...
	public function getAllData($dt){
		...
		$query = "SELECT tbl_student.name, tbl_attendance.*, classmst.*
				FROM tbl_student
				INNER JOIN tbl_attendance
				ON tbl_student.roll = tbl_attendance.roll
				INNER JOIN classmst
				ON tbl_student.classid =classmst.classid
				WHERE att_time = '$date'";
		$result = $this->db->select($query);
		return $result;
	}
